...
public class PlayerController : SingletonMono<PlayerController>
{
    ...
    private void Update()
    {
        ...
        {
            MusicMgr.Instance.PlaySound("跳跃", false);
        ...
        if (Input.GetKeyDown(KeyCode.E))
        {
            ...
            if (haveMonster)
            {
                if (weadMonster != null)
                {
                    ...
                    weadMonster = null;
                    MusicMgr.Instance.PlaySound("扔东西", false);
                }
            }
            else
            {
                ...
                if (weadMonster != null)
                {
                    ...
                    MusicMgr.Instance.PlaySound("拾起", false);
                    haveMonster = true;
                }
            }
        }
    ...
    public void DoDie()
    {
        MusicMgr.Instance.PlaySound("触碰死亡", false);
        coll.isTrigger = true;
        rb.simulated = false;
    ...
    void OnCollisionEnter2D(Collision2D other)
    {
        if (other.transform.tag == "Monster")
        {
            ...
            MusicMgr.Instance.PlaySound("触碰怪物", false);
            Die();
            return;
        }
